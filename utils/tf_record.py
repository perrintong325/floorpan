...
def load_raw_images(path):
	...
	image = imageio.imread(paths[0], pilmode='RGB')
	wall  = imageio.imread(paths[1], pilmode='L')
	close = imageio.imread(paths[2], pilmode='L')
	room  = imageio.imread(paths[3], pilmode='RGB')
	close_wall = imageio.imread(paths[4], pilmode='L')
	...
	wall = np.array(Image.fromarray(wall).resize((512, 512)))
	close = np.array(Image.fromarray(close).resize((512, 512)))
	close_wall = np.array(Image.fromarray(close_wall).resize((512, 512)))
	room = np.array(Image.fromarray(room).resize((512, 512, 3)))
...
def load_seg_raw_images(path):
	...
	# NOTE: imresize will rescale the image to range [0, 255], also cast data into uint8 or uint32
	image = np.array(Image.fromarray(image).resize((512, 512, 3)))
	close = np.array(Image.fromarray(close).resize((512, 512))) / 255
	close_wall = np.array(Image.fromarray(close_wall).resize((512, 512))) / 255
	room = np.array(Image.fromarray(room).resize((512, 512, 3)))
