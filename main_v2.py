...
class MODEL(Network):
	...
	def infer(self, save_dir='out', resize=True, merge=True):
		...
		for p in paths:
			im = imageio.imread(p, pilmode='RBG')  
			im_x = np.array(Image.fromarray(im).resize((512,512,3))) / 255. # resize and normalize
			...
			if resize:
				...
				out1_rgb = np.array(Image.fromarray(out1_rgb).resize((im.shape[0], im.shape[1]))) # resize back 
				out2_rgb = ind2rgb(np.squeeze(out2), color_map=floorplan_boundary_map)
				out2_rgb = np.array(Image.fromarray(out2_rgb).resize((im.shape[0], im.shape[1]))) # resize back 
			else:
				out1_rgb = ind2rgb(np.squeeze(out1))
				out2_rgb = ind2rgb(np.squeeze(out2), color_map=floorplan_boundary_map)
			...
			# imageio.imwrite(save_path4, out4)
	...
	def evaluate(self, sess, epoch, num_of_classes=11):
		...
		for i in xrange(n):
			im = imread(image_paths[i], pilmode='RBG')
			# for fuse label
			dd = imread(gt2_paths[i], pilmode='L')
			rr = imread(gt3_paths[i], pilmode='RBG')
			cw = imread(gt4_paths[i], pilmode='L')

			im = np.array(Image.fromarray(im).resize((512,512,3))) / 255.
			im = np.reshape(im, (1,512,512,3))
			# merge label
			rr = np.array(Image.fromarray(rr).resize((512,512,3)))
			rr_ind = rgb2ind(rr)
			cw = np.array(Image.fromarray(cw).resize((512,512))) / 255
			dd = np.array(Image.fromarray(dd).resize((512,512))) / 255
